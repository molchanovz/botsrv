...
db:
	@dropdb --if-exists botsrv
	@createdb botsrv
	@psql -f docs/botsrv.sql botsrv
	@psql -f docs/init.sql botsrv
...
mfd-xml:
	@mfd-generator xml -c "postgres://postgres:postgres@localhost:5432/DB_NAME?sslmode=disable" -m ./docs/model/botsrv.mfd -n $(MAPPING)
mfd-model:
	@mfd-generator model -m ./docs/model/botsrv.mfd -p db -o ./pkg/db
mfd-repo: --check-ns
	@mfd-generator repo -m ./docs/model/botsrv.mfd -p db -o ./pkg/db -n $(NS)
mfd-vt-xml:
	@mfd-generator xml-vt -m ./docs/model/botsrv.mfd
mfd-vt-rpc: --check-ns
	@mfd-generator vt -m docs/model/botsrv.mfd -o pkg/vt -p vt -x botsrv/pkg/db -n $(NS)
mfd-xml-lang:
...
mfd-vt-template: --check-ns type-script-client
	@mfd-generator template -m docs/model/botsrv.mfd  -o ../gold-vt/ -n $(NS)
