...
func (s AuthService) Logout(ctx context.Context) (bool, error) {
	...
	if ok, err := s.commonRepo.AuthenticateUser(ctx, user, nil); err != nil || !ok {
...
func (s AuthService) generateAuthKey(u *db.User, remember bool) *string {
	if remember {
		...
		return botsrv.Ptr(fmt.Sprintf("%x", md5.Sum([]byte(key))))
	}
	return botsrv.Ptr(s.generateRandom(32))
}
...
// GetByID returns a User by its Title.
//
//zenrpc:id int
//zenrpc:return User
//zenrpc:500 Internal Error
...
// Delete deletes the User by its Title.
//
//zenrpc:id int
...
func (s UserService) isValid(ctx context.Context, user User, isUpdate bool) Validator {
	...
	// check login unique
	item, err := s.commonRepo.OneUser(ctx, &db.UserSearch{Login: &user.Login})
	if err != nil {
